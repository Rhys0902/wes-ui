    ...
    <div class="p-2">
        ...
        <el-card shadow="hover">
            <el-row :gutter="10" class="mb8">
                <el-col :span="1.5">
                    <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['transfer:transferDetail:add']">新增</el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate" v-hasPermi="['transfer:transferDetail:edit']">修改</el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['transfer:transferDetail:remove']">删除</el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['transfer:transferDetail:export']">导出</el-button>
                </el-col>
            ...
            <el-table v-loading="loading" :data="transferDetailList" @selection-change="handleSelectionChange">
                ...
                <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200px">
                    <template #default="scope">
                        ...
                        <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['transfer:transferDetail:remove']">删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-dialog :title="title" v-model="open" width="500px" append-to-body>
            <el-form ref="transferDetailRef" :model="form" :rules="rules" label-width="80px">
                <el-form-item label="订单头ID" prop="headerId">
...
<script setup name="TransferDetail">
import { listTransferDetail, getTransferDetail, delTransferDetail, addTransferDetail, updateTransferDetail } from "@/api/transfer/transferDetail"

const { proxy } = getCurrentInstance()
const showAllSearchTop = ref(false)

const transferDetailList = ref([])
...
function getList() {
    ...
    listTransferDetail(queryParams.value).then(response => {
        transferDetailList.value = response.rows
...
function reset() {
    ...
    proxy.resetForm("transferDetailRef")
}
...
function handleUpdate(row) {
    ...
    getTransferDetail(_id).then(response => {
        loading.value = false
...
/** 提交按钮 */
function submitForm() {
    proxy.$refs["transferDetailRef"].validate(valid => {
        if (valid) {
            buttonLoading.value = true
            if (form.value.id != null) {
                updateTransferDetail(form.value)
                    .then(response => {
...
function handleDelete(row) {
    ...
    proxy.$modal
        .confirm('是否确认删除移库明细编号为"' + _ids + '"的数据项？')
        .then(function () {
            loading.value = true
            return delTransferDetail(_ids)
        })
        .then(() => {
            loading.value = true
            getList()
...
function handleExport() {
    proxy.download(
        "transfer/transferDetail/export",
        {
            ...queryParams.value
        },
        `移库明细_${new Date().getTime()}.xlsx`
    )
}
