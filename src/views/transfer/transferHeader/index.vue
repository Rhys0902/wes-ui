    ...
    <div class="p-2">
        <div v-show="showSearch" class="mb-[10px]">
            <el-card shadow="hover">
                <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="90px">
                    <el-form-item label="移库单号" prop="docNumber">
                        <el-input v-model="queryParams.docNumber" placeholder="请输入移库单号" clearable @keyup.enter="handleQuery" />
                    </el-form-item>
        ...
        <el-card shadow="hover">
            <el-row :gutter="10" class="mb8">
                <el-col :span="1.5">
                    <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['transfer:transferHeader:add']">新增</el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate" v-hasPermi="['transfer:transferHeader:edit']">修改</el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['transfer:transferHeader:remove']">删除</el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['transfer:transferHeader:export']">导出</el-button>
                </el-col>
                <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
            </el-row>

            <el-table v-loading="loading" :data="transferHeaderList" @selection-change="handleSelectionChange">
                <el-table-column type="selection" width="55" align="center" />
                <el-table-column label="主键ID" align="center" prop="id" v-if="false" />
                <el-table-column label="移库单号" align="center" prop="docNumber" />
                ...
                <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200px">
                    <template #default="scope">
                        <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['transfer:transferHeader:edit']">修改</el-button>
                        <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['transfer:transferHeader:remove']">删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-dialog :title="title" v-model="open" width="500px" append-to-body>
            <el-form ref="transferHeaderRef" :model="form" :rules="rules" label-width="80px">
                <el-form-item label="移库单号" prop="docNumber">
                    <el-input v-model="form.docNumber" placeholder="请输入移库单号" />
                </el-form-item>
                <el-form-item label="单据类型ID" prop="docTypeId">
...
<script setup name="TransferHeader">
import { listTransferHeader, getTransferHeader, delTransferHeader, addTransferHeader, updateTransferHeader } from "@/api/transfer/transferHeader"

const { proxy } = getCurrentInstance()
const showAllSearchTop = ref(false)

const transferHeaderList = ref([])
...
function getList() {
    ...
    listTransferHeader(queryParams.value).then(response => {
        transferHeaderList.value = response.rows
...
function reset() {
    ...
    proxy.resetForm("transferHeaderRef")
}
...
function handleUpdate(row) {
    ...
    getTransferHeader(_id).then(response => {
        loading.value = false
...
/** 提交按钮 */
function submitForm() {
    proxy.$refs["transferHeaderRef"].validate(valid => {
        if (valid) {
            buttonLoading.value = true
            if (form.value.id != null) {
                updateTransferHeader(form.value)
                    .then(response => {
...
function handleDelete(row) {
    ...
    proxy.$modal
        .confirm('是否确认删除移库订单头编号为"' + _ids + '"的数据项？')
        .then(function () {
            loading.value = true
            return delTransferHeader(_ids)
        })
        .then(() => {
            loading.value = true
            getList()
...
function handleExport() {
    proxy.download(
        "transfer/transferHeader/export",
        {
            ...queryParams.value
        },
        `移库订单头_${new Date().getTime()}.xlsx`
    )
}
