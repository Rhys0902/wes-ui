    ...
    <div class="p-2">
            ...
            <el-card shadow="hover">
                <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="90px">
                    ...
                    <span v-show="showAllSearchTop">
                        ...
                        <el-form-item label="库位ID" prop="locId">
                            <el-input v-model="queryParams.locId" placeholder="请输入库位ID" clearable @keyup.enter="handleQuery" />
                        </el-form-item>
                        <el-form-item label="库位编码" prop="locCode">
                            <el-input v-model="queryParams.locCode" placeholder="请输入库位编码" clearable @keyup.enter="handleQuery" />
                        </el-form-item>
        ...
        <el-card shadow="hover">
            <el-row :gutter="10" class="mb8">
                <el-col :span="1.5">
                    <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['count:countDetail:add']">新增</el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate" v-hasPermi="['count:countDetail:edit']">修改</el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['count:countDetail:remove']">删除</el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['count:countDetail:export']">导出</el-button>
                </el-col>
                <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
            </el-row>

            <el-table v-loading="loading" :data="countDetailList" @selection-change="handleSelectionChange">
                ...
                <el-table-column label="库位ID" align="center" prop="locId" />
                <el-table-column label="库位编码" align="center" prop="locCode" />
                <el-table-column label="容器ID" align="center" prop="leId" />
                ...
                <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200px">
                    <template #default="scope">
                        <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['count:countDetail:edit']">修改</el-button>
                        <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['count:countDetail:remove']">删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-dialog :title="title" v-model="open" width="500px" append-to-body>
            <el-form ref="countDetailRef" :model="form" :rules="rules" label-width="80px">
                <el-form-item label="订单头ID" prop="headerId">
                ...
                <el-form-item label="库位ID" prop="locId">
                    <el-input v-model="form.locId" placeholder="请输入库位ID" />
                </el-form-item>
                <el-form-item label="库位编码" prop="locCode">
                    <el-input v-model="form.locCode" placeholder="请输入库位编码" />
                </el-form-item>
                <el-form-item label="容器ID" prop="leId">
                    <el-input v-model="form.leId" placeholder="请输入容器ID" />
                </el-form-item>
                <el-form-item label="容器编码" prop="leCode">
                    <el-input v-model="form.leCode" placeholder="请输入容器编码" />
                </el-form-item>
                <el-form-item label="区域ID" prop="areaId">
...
import { listCountDetail, getCountDetail, delCountDetail, addCountDetail, updateCountDetail } from "@/api/count/countDetail"
...
function reset() {
    ...
    proxy.resetForm("countDetailRef")
}
...
function handleUpdate(row) {
    ...
    getCountDetail(_id).then(response => {
        loading.value = false
...
/** 提交按钮 */
function submitForm() {
    proxy.$refs["countDetailRef"].validate(valid => {
        if (valid) {
            buttonLoading.value = true
            if (form.value.id != null) {
                updateCountDetail(form.value)
                    .then(response => {
...
function handleDelete(row) {
    ...
    proxy.$modal
        .confirm('是否确认删除盘点明细编号为"' + _ids + '"的数据项？')
        .then(function () {
            loading.value = true
            return delCountDetail(_ids)
        })
        .then(() => {
            loading.value = true
            getList()
...
function handleExport() {
    proxy.download(
        "count/countDetail/export",
        {
            ...queryParams.value
        },
        `盘点明细_${new Date().getTime()}.xlsx`
    )
}
