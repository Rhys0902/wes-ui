    ...
    <div class="p-2">
        ...
        <el-card shadow="hover">
            <el-row :gutter="10" class="mb8">
                <el-col :span="1.5">
                    <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['bas:basMaterialType:add']">新增</el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate" v-hasPermi="['bas:basMaterialType:edit']">修改</el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['bas:basMaterialType:remove']">删除</el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['bas:basMaterialType:export']">导出</el-button>
                </el-col>
                <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
            </el-row>

            <el-table v-loading="loading" :data="basMaterialTypeList" @selection-change="handleSelectionChange" :default-sort="{prop: 'id', order: 'ascending '}" @sort-change="handleSortChange">
                ...
                <!-- <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200px" fixed="right">
                    <template #default="scope">
                        ...
                        <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['bas:basMaterialType:remove']">删除</el-button>
                    </template>
                </el-table-column> -->
            </el-table>
        ...
        <el-dialog :title="title" v-model="open" width="500px" append-to-body>
            <el-form ref="basMaterialTypeRef" :model="form" :rules="rules" label-width="120px">
                <el-form-item label="物料类型编码" prop="materialTypeCode">
...
<script setup name="BasMaterialType">
import { listBasMaterialType, getBasMaterialType, delBasMaterialType, addBasMaterialType, updateBasMaterialType } from "@/api/bas/basMaterialType"

const { proxy } = getCurrentInstance()
const showAllSearchTop = ref(false)

const basMaterialTypeList = ref([])
...
const data = reactive({
    ...
    rules: {
        materialTypeCode: [{ required: true, message: "请输入", trigger: "blur" }],
        materialTypeName: [{ required: true, message: "请输入", trigger: "blur" }]
    }
})
...
function getList() {
    ...
    listBasMaterialType(queryParams.value).then(response => {
        basMaterialTypeList.value = response.rows
...
function reset() {
    ...
    proxy.resetForm("basMaterialTypeRef")
}
...
function handleUpdate(row) {
    ...
    getBasMaterialType(_id).then(response => {
        loading.value = false
...
/** 提交按钮 */
function submitForm() {
    proxy.$refs["basMaterialTypeRef"].validate(valid => {
        if (valid) {
            buttonLoading.value = true
            if (form.value.id != null) {
                updateBasMaterialType(form.value)
                    .then(response => {
...
function handleDelete(row) {
    ...
    proxy.$modal
        .confirm('是否确认删除物料类型编号为"' + _ids + '"的数据项？')
        .then(function () {
            loading.value = true
            return delBasMaterialType(_ids)
        })
        .then(() => {
            loading.value = true
            getList()
...
function handleExport() {
    proxy.download(
        "bas/basMaterialType/export",
        {
            ...queryParams.value
        },
        `物料类型${new Date().getTime()}.xlsx`
    )
}
