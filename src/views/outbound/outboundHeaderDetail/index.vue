    ...
    <div class="p-2">
        ...
        <el-card shadow="hover">
            <el-row :gutter="10" class="mb8">
                <el-col :span="1.5">
                    <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['outbound:outboundHeaderDetail:add']">新增</el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate" v-hasPermi="['outbound:outboundHeaderDetail:edit']">修改</el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['outbound:outboundHeaderDetail:remove']">删除</el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['outbound:outboundHeaderDetail:export']">导出</el-button>
                </el-col>
                <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
            </el-row>

            <el-table v-loading="loading" :data="outboundHeaderDetailList" @selection-change="handleSelectionChange">
                <!-- <el-table-column type="selection" width="55" align="center" /> -->
                <el-table-column label="出库单号" align="center" prop="docNumber" />
                ...
                <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200px">
                    <template #default="scope">
                        <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['outbound:outboundHeaderDetail:edit']">修改</el-button>
                        <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['outbound:outboundHeaderDetail:remove']">删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-dialog :title="title" v-model="open" width="500px" append-to-body>
            <el-form ref="outboundHeaderDetailRef" :model="form" :rules="rules" label-width="80px">
                <el-form-item label="出库单号" prop="docNumber">
                    <el-input v-model="form.docNumber" placeholder="请输入出库单号" />
                </el-form-item>
                <el-form-item label="单据类型ID" prop="docTypeId">
...
<script setup name="OutboundHeaderDetail">
import {
    listOutboundHeaderDetail,
    getOutboundHeaderDetail,
    delOutboundHeaderDetail,
    addOutboundHeaderDetail,
    updateOutboundHeaderDetail
} from "@/api/outbound/outboundHeaderDetail"

const { proxy } = getCurrentInstance()
const showAllSearchTop = ref(false)

const outboundHeaderDetailList = ref([])
...
function getList() {
    ...
    listOutboundHeaderDetail(queryParams.value).then(response => {
        outboundHeaderDetailList.value = response.rows
...
function reset() {
    ...
    proxy.resetForm("outboundHeaderDetailRef")
}
...
function handleUpdate(row) {
    ...
    getOutboundHeaderDetail(_headerId).then(response => {
        loading.value = false
...
/** 提交按钮 */
function submitForm() {
    proxy.$refs["outboundHeaderDetailRef"].validate(valid => {
        if (valid) {
            buttonLoading.value = true
            if (form.value.headerId != null) {
                updateOutboundHeaderDetail(form.value)
                    .then(response => {
                ...
                addOutboundHeaderDetail(form.value)
                    .then(response => {
...
function handleDelete(row) {
    ...
    proxy.$modal
        ...
        .then(function () {
            loading.value = true
            return delOutboundHeaderDetail(_headerIds)
        })
        .then(() => {
            loading.value = true
            getList()
...
function handleExport() {
    proxy.download(
        "outbound/outboundHeaderDetail/export",
        {
            ...queryParams.value
        },
        `outboundHeaderDetail_${new Date().getTime()}.xlsx`
    )
}
