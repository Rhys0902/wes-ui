    ...
    <div class="p-2">
        ...
        <el-card shadow="hover">
            <el-row :gutter="10" class="mb8">
                <el-col :span="1.5">
                    <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['repl:replTrans:add']">新增</el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate" v-hasPermi="['repl:replTrans:edit']">修改</el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['repl:replTrans:remove']">删除</el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['repl:replTrans:export']">导出</el-button>
                </el-col>
                <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
            </el-row>

            <el-table v-loading="loading" :data="replTransList" @selection-change="handleSelectionChange">
                ...
                <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200px">
                    <template #default="scope">
                        <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['repl:replTrans:edit']">修改</el-button>
                        <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['repl:replTrans:remove']">删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-dialog :title="title" v-model="open" width="500px" append-to-body>
            <el-form ref="replTransRef" :model="form" :rules="rules" label-width="80px">
                <el-form-item label="订单ID" prop="docId">
...
<script setup name="ReplTrans">
import { listReplTrans, getReplTrans, delReplTrans, addReplTrans, updateReplTrans } from "@/api/repl/replTrans"

const { proxy } = getCurrentInstance()
const showAllSearchTop = ref(false)

const replTransList = ref([])
...
function getList() {
    ...
    listReplTrans(queryParams.value).then(response => {
        replTransList.value = response.rows
...
function handleUpdate(row) {
    ...
    getReplTrans(_id).then(response => {
        loading.value = false
...
/** 提交按钮 */
function submitForm() {
    proxy.$refs["replTransRef"].validate(valid => {
        if (valid) {
            buttonLoading.value = true
            if (form.value.id != null) {
                updateReplTrans(form.value)
                    .then(response => {
...
function handleDelete(row) {
    ...
    proxy.$modal
        .confirm('是否确认删除补货交易编号为"' + _ids + '"的数据项？')
        .then(function () {
            loading.value = true
            return delReplTrans(_ids)
        })
        .then(() => {
            loading.value = true
            getList()
...
function handleExport() {
    proxy.download(
        "repl/replTrans/export",
        {
            ...queryParams.value
        },
        `补货交易_${new Date().getTime()}.xlsx`
    )
}
